...
export default class RankManager extends cc.Component {
        ...
        try {
            ...
            if (result.code === 0 && result.data) {
                ...
                this.rankData = result.data.map((item: any) => {
                    const highestPassedLevel = Math.max(0, Math.floor(Number(item.totalLoginNum) || 0));
                    return {
                        rank: 0,
                        name: `玩家${item.accountId}`,
                        level: Math.max(1, Math.min(totalLevels, highestPassedLevel + 1))
                    };
                });
    ...
    private showSelfInfo(): void {
        ...
        const localUnlockedLevel = mGameData.getHighestUnlockedLevel
