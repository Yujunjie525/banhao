...
export default class ShopManager extends cc.Component {
    ...
    private roleShopSummaries: {[roleIndex: number]: string} = {};
    ...
    private loadRoleShopSummaries() {
        cc.loader.loadRes('config/characterTraits', cc.JsonAsset, (err: Error, jsonAsset: cc.JsonAsset) => {
            if (err || !jsonAsset) {
                console.error('加载角色能力描述配置失败:', err);
                return;
            }

            const config: any = jsonAsset.json || {};
            const characters = Array.isArray(config) ? config : config.characters;
            if (!Array.isArray(characters)) return;

            this.roleShopSummaries = {};
            characters.forEach((character: any) => {
                if (!character || character.enabled === false) return;
                const roleIndex = Math.floor(Number(character.role_index));
                const summary = typeof character.shop_summary === 'string'
                    ? character.shop_summary.trim()
                    : '';
                if (!isNaN(roleIndex) && roleIndex >= 0) {
                    this.roleShopSummaries[roleIndex] = summary;
                }
            });

            if (this.node && this.node.isValid) this.initShopItems(true);
        });
    }
    ...
    initShopItems(keepScroll: boolean = false) {
        ...
        for (let i = 0; i < mGameData.rolePrices.length; i++) {
            this.shopItems.push({
                type: 'role',
                index: i,
                summary: Object.prototype.hasOwnProperty.call(this.roleShopSummaries, i)
                    ? this.roleShopSummaries[i]
                    : (i === 0 ? '' : '角色能力'),
                iconPath: `zzImg2/${i + 1}`,
    ...
    setItemData(itemNode: cc.Node, itemData: any) {
        ...
        if (iconNode) {
            ...
            if (spriteComponent) {
                if (itemData.type === 'role') {
                    ...
                    const iconPath = itemData.iconPath || `zzImg2/${itemData.index + 1}`;
                    iconNode.scale = 1;
                    ...
                    // 体力商品，显示 tili.png
                    cc.loader.loadRes('zzImg/tili', cc.SpriteFrame, (err, spriteFrame) => {
                        if (err) {
        ...
        if (itemData.type === 'role') {
            ...
            // 名称位置改为展示配置中的角色能力概述。
            if (nameNode) {
                const summaryLabel = nameNode.getComponent(cc.Label);
                if (summaryLabel) {
                    const summary = itemData.summary || '';
                    nameNode.setContentSize(245, 36);
                    nameNode.setPosition(nameNode.x, -37.367);
                    summaryLabel.fontSize = 20;
                    summaryLabel.lineHeight = 22;
                    summaryLabel.enableWrapText = false;
                    summaryLabel.overflow = cc.Label.Overflow.CLAMP;
    ...
    private refreshRoleSelectionState() {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            onNode.active = true;
            const textNode = onNode.getChildByName('text');
            const label = textNode ? textNode.getComponent(cc.Label) : null;
            if (label) {
