...
export default class SplashManager extends cc.Component {
        ...
        if (this.username && this.password) {
            ...
            if (globalRealName === 'true') {
                ...
                try {
                    ...
                    this.applyServerLevelProgress(result.data, result.data.accountId || result.data.user_id);
                    ...
                    try {
                        ...
                        if (antiAddictionResult.data && antiAddictionResult.code === -1) {
                            // 被拦截后清除自动登录凭据，下一次启动必须回到可输入的登录界面。
                            this.clearSavedLoginCredentials();
                            this.showLoginPanel();
    ...
    async onLoginClick() {
        ...
        try {
            ...
            if (result.code === 0) {
                ...
                this.applyServerLevelProgress(result.data, userId);
                 if (result.data.foam && result.data.used_foam) {
                    // 计算道具1的库存：总数量 - 已使用数量
                    const item1Stock = result.data.foam - result.data.used_foam;
                    // 计算道具2的库存：总数量 - 已使用数量
    ...
    private applyServerLevelProgress(data: any, userId: any): void {
        if (!data) return;
        if (mGameData.GetLevelData) mGameData.GetLevelData();

        const totalLevels = mGameData.getTotalLevels ? mGameData.getTotalLevels() : 100;
        let highestPassedLevel = Math.max(0, Math.floor(Number(data.rank) || 0));
        const stars = Array.isArray(mGameData.levelStars) ? mGameData.levelStars.slice() : [];
        const ranklist = Array.isArray(data.ranklist) ? data.ranklist : [];
        ranklist.forEach((levelData: any) => {
            if (!levelData) return;
            const level = Math.floor(Number(levelData.rank) || 0);
            if (level < 1 || level > totalLevels) return;
            highestPassedLevel = Math.max(highestPassedLevel, level);
            while (stars.length < level) stars.push(0);
            const star = Math.max(0, Math.min(3, Math.floor(Number(levelData.star) || 0)));
            stars[level - 1] = Math.max(Number(stars[level - 1]) || 0, star);
        });

        if (highestPassedLevel <= 0) return;
        const currentLevel = Math.min(totalLevels, highestPassedLevel);
        const unlockedLevel = Math.min(totalLevels, highestPassedLevel + 1);
        mGameData.currentLevel = Math.max(mGameData.currentLevel || 1, currentLevel);
        mGameData.unlockedLevel = Math.max(mGameData.unlockedLevel || 1, unlockedLevel);
        mGameData.levelStars = stars;
        if (mGameData.SaveLevelData) mGameData.SaveLevelData();

        cc.sys.localStorage.setItem('CurrentLevel', String(mGameData.currentLevel));
        cc.sys.localStorage.setItem('UnlockedLevel', String(mGameData.unlockedLevel));
        console.log('已合并服务端关卡进度:', {
            userId,
            highestPassedLevel,
            unlockedLevel: mGameData.unlockedLevel
        });
    }

    private clearSavedLoginCredentials() {
