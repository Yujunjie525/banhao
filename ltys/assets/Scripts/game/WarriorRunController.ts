...
type EntityType = 'gold' | 'energy' | 'obstacle';
...
export default class WarriorRunController extends cc.Component {
    ...
    private dimensionCooldownNode: cc.Node = null;
    private dimensionCooldownLabel: cc.Label = null;
    ...
    update(dt: number) {
        ...
        if (this.weatherTimer >= WEATHER_SWITCH_SECONDS) {
            this.weatherTimer = 0;
            this._switchWeather();
        }
    ...
    private _buildHUD() {
        ...
        this.timeLeftLabel = this._findOrCreateEditableLabel('timeLeftLabel', this.hudLayer, '第1关 00：30', 24, new cc.Color(255, 232, 92, 255), cc.v2(0, 495));
    ...
    private _ensureDimensionButtonUI() {
        ...
        this.dimensionCooldownNode = this.dimensionBtn.getChildByName('CooldownMask');
        if (!this.dimensionCooldownNode) {
            this.dimensionCooldownNode = this._node('CooldownMask', this.dimensionBtn, this.dimensionBtn.width, this.dimensionBtn.height);
        }
        this.dimensionCooldownNode.setPosition(0, 0);
        this.dimensionCooldownNode.setContentSize(this.dimensionBtn.width, this.dimensionBtn.height);
        this.dimensionCooldownNode.zIndex = 8;
        this.dimensionCooldownNode.active = false;

        this._ensureCooldownMaskVisual();

        let cooldownLabelNode = this.dimensionCooldownNode.getChildByName('CdLabel');
        if (!cooldownLabelNode) {
            cooldownLabelNode = new cc.Node('CdLabel');
            this.dimensionCooldownNode.addChild(cooldownLabelNode);
            cooldownLabelNode.setPosition(0, 0);
        }
        cooldownLabelNode.zIndex = 12;
        cooldownLabelNode.color = cc.Color.WHITE;
        this.dimensionCooldownLabel = cooldownLabelNode.getComponent(cc.Label);
        if (!this.dimensionCooldownLabel) this.dimensionCooldownLabel = cooldownLabelNode.addComponent(cc.Label);
        this.dimensionCooldownLabel.fontSize = 30;
        this.dimensionCooldownLabel.lineHeight = 34;
        this.dimensionCooldownLabel.horizontalAlign = cc.Label.HorizontalAlign.CENTER;
        this.dimensionCooldownLabel.verticalAlign = cc.Label.VerticalAlign.CENTER;
    }

    // 处理冷却遮罩显示：优先使用编辑器里配好的 Sprite，没有才画兜底圆形。
    private _ensureCooldownMaskVisual() {
        if (!this.dimensionCooldownNode) return;
        const sprite = this.dimensionCooldownNode.getComponent(cc.Sprite);
        let graphics = this.dimensionCooldownNode.getComponent(cc.Graphics);

        if (sprite && sprite.spriteFrame) {
            sprite.enabled = false;
        }

        if (!graphics) graphics = this.dimensionCooldownNode.addComponent(cc.Graphics);
    ...
    private _startRun() {
        ...
        this.distance = 0;
        this.gold = 0;
        this.energy = Math.min(60, this._getEnergyCap());
        this.runTime = 0;
        this.spawnMeter = 0;
        this.weatherTimer = 0;
    ...
    // 按跑动距离触发一波实体生成，决定这一波是钻石、钻石堆、能量还是障碍。
    private _spawnEntities() {
        ...
        const totalWeight = weights.goldLine + weights.goldPile + weights.energy + weights.obstacle;
    ...
    private _createEntityNode(type: EntityType): cc.Node {
        const templateName = type === 'gold' ? 'GoldTemplate' : type === 'energy' ? 'EnergyTemplate' : 'ObstacleTemplate';
        const template = this.entityTemplates && this.entityTemplates.getChildByName(templateName);
        if (template) {
        ...
        return this._node(type, this.entityLayer, type === 'obstacle' ? 118 : 54, type === 'obstacle' ? 78 : 54);
    }
    ...
    // 角色4吸附逻辑：吸附视觉所在跑道及相邻跑道、5米范围内钻石/能量。
    ...
    private _playDimensionButtonDenied() {
        ...
        this.dimensionBtn.stopAllActions();
        this.dimensionBtn.color = new cc.Color(220, 70, 70, 255);
        this.dimensionBtn.runAction(cc.sequence(
            cc.scaleTo(0.06, 0.92),
            cc.scaleTo(0.08, 1),
            cc.callFunc(() => this.dimensionBtn.color = cc.Color.WHITE)
        ));
    }
    ...
    private _updateWeatherEffects(dt: number) {
        ...
        const warning = this.weatherTimer >= WEATHER_SWITCH_SECONDS - WEATHER_WARNING_SECONDS;
    ...
    private _completeLevel() {
        ...
        if (shouldReportPass) {
            UserDataSyncManager.postPassLevel(this.currentLevelId, 3).catch((error) => {
    ...
    private _showLosePopup() {
        ...
        this._showPopup('Game Over', '成绩: ' + Math.floor(this.distance) + '米', 'Gold: ' + this.gold);
    }

    private _showWinPopup() {
        if (this.winResultPopup) {
            this._showVisualPopup(this.winResultPopup);
            return;
        }
        this._showPopup('Level Clear', '第' + this.currentLevelId + '关完成', 'Gold: ' + this.gold);
    }
    ...
    // 保存本局距离和钻石。unlockOnly=true 时只保存距离，不把本局钻石加到账户。
    private _saveRunResult(unlockOnly: boolean, levelPassed = false): boolean {
        ...
        if (levelPassed) {
            ...
            p.level_stars[String(level)] = Math.max(3, Math.floor(Number(p.level_stars[String(level)]) || 0));
            p.unlocked_level = Math.max(
                previousUnlockedLevel,
                Math.min(getWarriorRunTotalLevels(), level + 1)
            );
            GameState.lastStars = 3;
    ...
    private _updateHUD() {
        ...
        const weatherCountdown = WEATHER_SWITCH_SECONDS - this.weatherTimer;
        ...
        if (this.dimensionCooldownNode) {
            this.dimensionCooldownNode.active = this.dimension !== 'qin' || this.dimensionCooldownTimer > 0;
            this._updateDimensionCooldownMask();
        }
        if (this.dimensionCooldownLabel) {
            this.dimensionCooldownLabel.string = this.dimension === 'qin' && this.dimensionCooldownTimer > 0 ? this.dimensionCooldownTimer.toFixed(1) : '';
        }
        if (this.dimensionBtn) {
    ...
    private _updateDimensionCooldownMask() {
        if (!this.dimensionCooldownNode) return;
        const sprite = this.dimensionCooldownNode.getComponent(cc.Sprite);
        ...
        if (this.dimension !== 'qin') {
            const radius = Math.min(this.dimensionCooldownNode.width, this.dimensionCooldownNode.height) * 0.5;
        ...
        const duration = Math.max(0.01, this._getDimensionCooldownSeconds());
        const progress = Math.max(0, Math.min(1, this.dimensionCooldownTimer / duration));
        ...
        const segments = Math.max(6, Math.ceil(48 * progress));
        graphics.moveTo(0, 0);
