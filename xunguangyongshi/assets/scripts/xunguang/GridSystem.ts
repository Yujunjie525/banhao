...
import { PusherAnimator } from './PusherAnimator';
...
export class GridSystem extends Component {
    ...
    public updatePusherBehind(block: LightBlock, direction: GridDirection, coord: GridCoord) {
        ...
        this.getPusherAnimator()?.setDirectionByDelta(direction.col, direction.row);
    }
    ...
    public async hidePusher() {
        const animator = this.getPusherAnimator();
        if (!animator || !animator.isVisible()) {
            if (this.pusherNode) {
    ...
    private ensurePusher() {
        ...
        if (!this.pusherNode || !this.pusherNode.isValid) {
            ...
            this.ensurePusherAnimator();
        }

        return this.pusherNode;
    }

    public async movePusherToBlockAndShow(block: LightBlock, direction: GridDirection, coord: GridCoord) {
        const pusher = this.ensurePusher();
        if (!pusher) {
            return;
        }

        const animator = this.ensurePusherAnimator();
        const shouldHideFirst = animator?.isVisible();
        if (shouldHideFirst) {
            await this.hidePusher();
        }

        this.updatePusherBehind(block, direction, coord);
        pusher.active = true;
        animator?.setDirectionByDelta(direction.col, direction.row);
        await animator?.playShow();
    }

    public stopPusherImmediately() {
        this.getPusherAnimator()?.stopImmediately();
        if (this.pusherNode) {
            this.pusherNode.active = false;
        }
    ...
    public stopPusherAtCurrentSpot() {
        this.getPusherAnimator()?.stopWalkKeepVisible();
    }

    public startPusherWalk() {
        void this.getPusherAnimator()?.playWalk();
    }

    private ensurePusherAnimator() {
        if (!this.pusherNode || !this.pusherNode.isValid) {
            return null;
        }

        return this.pusherNode.getComponent(PusherAnimator) ?? this.pusherNode.addComponent(PusherAnimator);
    }

    private getPusherAnimator() {
        if (!this.pusherNode || !this.pusherNode.isValid) {
            return null;
        }

        return this.pusherNode.getComponent(PusherAnimator);
    }
    ...
    public getSlotCoords(): GridCoord[] {
        return Array.from(this.slots.values()).map((slot) => ({ ...slot.position }));
    }
