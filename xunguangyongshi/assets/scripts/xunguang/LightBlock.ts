...
export class LightBlock extends Component {
    ...
    public async trySlide(direction: GridDirection) {
        ...
        await this.grid.movePusherToBlockAndShow(this, direction, from);
        if (!this.node.isValid || !this.grid || !GameManager.instance?.canPlay()) {
            return;
        }
        this.isSliding = true;
    ...
    private slideAlongPath(path: GridCoord[], totalDuration: number) {
        if (path.length <= 1) {
            ...
            this.grid.stopPusherAtCurrentSpot();
            GameManager.instance?.onBlockMoved();
            return;
        }
    ...
    public rollbackToLastStableState() {
        ...
        this.grid.stopPusherImmediately();
        this.checkSlot();
        this.setSelected(false);
        this.needContinueRollback = false;
    }
