...
export class cloudUserData extends Component {
    ...
    private readonly USER_SCOPED_KEYS = new Set([
        'jinbiNum',
        'lightCrystal',
        'memoryFragment',
        'tiliNum',
    ...
    public isSyncableKey(key: string) {
        for (const syncKey of this.SYNC_KEYS) {
            if (key === this.getStorageKey(syncKey) && this.isUserScopedKey(syncKey)) {
                return true;
            }
        }
        ...
        return this.SYNC_KEY_PREFIXES.some((prefix) => {
    ...
    public listSyncableKeys() {
        const keys = [...this.SYNC_KEYS].map((key) => this.getStorageKey(key));
        const userId = this.getUserId();
        if (userId) {
            keys.push(`SLS_STAMINA_${userId}`, `SLS_RECOVER_TIME_${userId}`);
        }
        return keys;
    }
    ...
    private applyStorage(storage: Record<string, any>) {
        for (const key of Object.keys(storage)) {
            if (!this.SYNC_KEYS.has(key) && !this.isSyncableKey(key)) {
                continue;
            }

            const value = storage[key];
            const normalized = this.stringifyValue(value);
            sys.localStorage.setItem(this.getStorageKey(key), normalized);
        }
    }

    private isUserScopedKey(key: string) {
        return this.USER_SCOPED_KEYS.has(key);
    }

    private getStorageKey(key: string) {
        const userId = this.getUserId();
        if (!userId || !this.isUserScopedKey(key)) {
            return key;
        }
    ...
    private extractStorage(response: any): Record<string, any> | null {
        ...
        if (parsed?.version && parsed?.storage && typeof parsed.storage === 'object') {
