...
export default class ShopManager extends Component {
    ...
    private readonly defaultRoleNames = ['寻光学徒', '追风旅人', '晨曦祭司', '灵魂织补者', '空间架构师'];

    private readonly defaultRoleEffects = [
        '默认解锁',
        '额外步数',
        '地图亮度提升',
        '免疫噬梦黑洞',
    ...
    private buildShopItems() {
        ...
        for (let i = 0; i < roleRecords.length; i++) {
            items.push({
                ...
                effectText: this.defaultRoleEffects[i],
    ...
    private renderItem(itemNode: Node, data: ShopListItemData) {
        ...
        this.setChildLabel(itemNode, 'Label2', data.effectText || '', data.kind === 'role');
        this.setChildSprite(itemNode, 'juese1', data.iconSpriteFrame);
        this.setItemIconScale(itemNode, data.kind === 'stamina' ? 1 : 1);
    ...
    onRoleUnlockClick(roleIndex: number) {
        ...
        if (gameConfig.jinbiNum < role.price) {
            this.showTips('金币不足，无法解锁角色。');
            return;
        }

        this.showConfirmPanel('unlockRole', roleIndex, `确定要花费${role.price}金币解锁角色吗？`);
    }
    ...
    onStaminaBuyClick = () => {
        ...
        if (gameConfig.jinbiNum < this.staminaPrice) {
            this.showTips('金币不足，无法购买体力。');
            return;
        }

        this.showConfirmPanel('buyStamina', -1, `确定要花费${this.staminaPrice}金币购买${this.staminaAddAmount}点体力吗？`);
    };
    ...
    private executeRoleUnlock(roleIndex: number) {
        ...
        gameConfig.jinbiNum = Math.max(0, gameConfig.jinbiNum - role.price);
    ...
    private executeStaminaBuy() {
        ...
        gameConfig.jinbiNum = Math.max(0, gameConfig.jinbiNum - this.staminaPrice);
        save(localData.jinbiNum, gameConfig.jinbiNum);
    ...
    private setChildLabel(node: Node | null, childName: string, value: string, visible = true) {
