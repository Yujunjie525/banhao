...
export default class AchieveManager extends Component {
    ...
    private getHighestPassedLevel() {
        const nowLevel = Number(gameConfig.nowLevel || 1);
        return nowLevel - 1;
    }
    ...
    private adjustContainerSize() {
        ...
        const horizontalSpacing = 20; // 两个列表项之间的间隔（可调整）
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const x = (col * (itemWidth + actualHorizontalSpacing)) - (viewTransform.width / 2) + itemWidth / 2 + 10;
    ...
    private onClaimClick(achieveId: number) {
        ...
        this.refreshScrollView();
        this.showToast(`获得${achieve.rewardGold}金币。`);
    }
