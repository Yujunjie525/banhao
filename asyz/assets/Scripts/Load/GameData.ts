...
class GameData{
    ...
    public roleWeights: Array<number> = [4, 5, 6, 7, 8]; //兼容旧字段，角色实际移速以 ArcaneCharacterConfig 为准
    public rolePrices: Array<number> = [0, 2000, 4000, 6000, 8000]; //角色价格
    public roleNames: Array<string> = ['碧穹剑姬', '赤焰疾姬', '玄甲卫士', '炎灵法尊', '月影刺姬']; //角色名称
...
private static LEVEL_CONFIGS: LevelConfig[] = [
    ...
    private static buildLevelConfigs(): LevelConfig[] {
        const configs = GameData.LEVEL_CONFIGS.map((config) => ({
            ...config,
            icon: `ditupeitu${Math.min(5, Math.ceil(config.levelId / 20))}`
        }));
        for (let level = configs.length + 1; level <= 100; level++) {
            configs.push({
                levelId: level,
                icon: `ditupeitu${Math.min(5, Math.ceil(level / 20))}`,
                time: 60,
                Monster: [0, 1, 2, 3, 4],
                num: level * 2 + 10,
                speed: 2
    ...
    constructor() {
        // 直接使用静态关卡配置数组
        this.levelConfigs = GameData.buildLevelConfigs();
    ...
    GetBestScoreData(){
        const bestScoreKey = this.getKeyWithUserId(this.BestScoreKey);
        const bestScoreStr = cc.sys.localStorage.getItem(bestScoreKey);
    ...
    CheckAndRecoverStamina(){
        ...
        const recoverInterval = 6 * 60 * 1000; // 6分钟，单位毫秒
