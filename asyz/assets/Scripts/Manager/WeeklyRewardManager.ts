...
export default class WeeklyRewardManager extends cc.Component {
    ...
    private updateWeeklyRewardUI(preserveScrollPosition: boolean = false): void {
        if (!this.weekPanel) {
        ...
        } else if (this.rewardScrollView.scrollToTop) {
    ...
    private ensureRewardScrollList(): boolean {
        ...
        if (!this.rewardScrollView || !this.rewardContent || !this.rewardItemTemplate) {
            console.error('Weekly reward ScrollView, content or item template not assigned');
            return false;
        }
        ...
        if (!scrollNode || !viewNode) {
            console.error('Weekly reward ScrollView hierarchy is incomplete');
            return false;
        }
    ...
    private setRewardItemData(dayNode: cc.Node, rewardData: WeeklyRewardItem): void {
        ...
        if (labelNode) {
            ...
            if (label) {
                ...
                label.string = weekLabels[rewardData.id - 1] || rewardData.name;
            }
        }
    ...
    private onClaimClick(rewardId: number): void {
        ...
        this.updateWeeklyRewardUI(true);
