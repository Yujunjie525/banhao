...
export default class LoadManager extends cc.Component {
    ...
    private _resettingAccount: boolean = false;
    ...
    onLoad () {
        ...
        cc.director.preloadScene(Scene.ArcaneWarrior);
    ...
    updateLevelDisplay(){
        ...
        // GameState.maxUnlockedLevel 是逃离模式的最高关卡索引（从 0 开始）。
        const totalLevels = mGameData.getTotalLevels ? mGameData.getTotalLevels() : 100;
        const escapeUnlockedLevel = Math.max(1, Math.floor(GameState.maxUnlockedLevel || 0) + 1);
        const highestUnlockedLevel = Math.min(totalLevels, escapeUnlockedLevel);
     ...
     */
    async ResetAccount(){
        if (this._resettingAccount) return;
        this._resettingAccount = true;
        try {
            await UserDataSyncManager.flushUpload();
        } catch (error) {
            console.error('account switch sync failed, continuing:', error);
        }
    ...
    initStoryPopup(){
        ...
        // 初始化剧情文本数组（示例为十几行文本）
        this.storyLines = [
            "久远之前，先贤将浩瀚的奥术知识封藏于一座棋盘遗迹之中。这片由无数方格构筑的棋域，处处布下隐匿的魔法陷阱，用来考验前来探寻真相的冒险者。曾经守护遗迹的符文标记早已消散，危险潜伏在每一块格子之下。你是拥有奥术解析天赋的勇者，被命运召唤来到这片神秘棋盘。你无法直接看见隐藏的危机，唯有依靠解析能力感知危险的方位，追寻出口的踪迹。每一步抉择都暗藏凶险，稍有不慎便会触发陷阱。在交错的方格之间谨慎前行，避开无处不在的危机，穿越这片奥术棋域，抵达遗迹的出口，解锁被岁月掩埋的古老秘密。",
        ];
        ...
        // 清空当前文本
        if(this.r_story){
            this.r_story.node.color = cc.Color.WHITE;
            const outline = this.r_story.node.getComponent(cc.LabelOutline) || this.r_story.node.addComponent(cc.LabelOutline);
            outline.color = cc.color(10, 46, 101, 255);
            outline.width = 3;
            this.r_story.string = "";
        }
    ...
    private playBGMFromStart(){
        this.stopBGM();
        if(!mGameData.isBGMOn || !this.Bgm){
            return;
        }
        this._bgmAudioId = cc.audioEngine.play(this.Bgm, true, 1);
