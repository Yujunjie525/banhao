...
export default class StateBridge {
    ...
    static consumeStamina(): boolean {
        ...
        // Count only successful level-entry consumption for stamina achievements.
        UserDataSyncManager.recordConsumedStamina(1);
        GameState.stamina = Math.max(0, mGameData.currentStamina || 0);
        GameState.save();
        cc.director.emit('staminaUpdated');
        UserDataSyncManager.requestUpload();
    ...
    static hasEnoughStamina(): boolean {
        if (mGameData.GetStaminaData) mGameData.GetStaminaData();
        if (mGameData.CheckAndRecoverStamina) mGameData.CheckAndRecoverStamina();
        return mGameData.HasEnoughStamina
            ? mGameData.HasEnoughStamina()
            : (mGameData.currentStamina || 0) > 0;
    }

    static refundStamina(amount: number = 1): void {
        if (amount <= 0) return;
        if (mGameData.GetStaminaData) mGameData.GetStaminaData();
        if (mGameData.CheckAndRecoverStamina) mGameData.CheckAndRecoverStamina();
        const maxStamina = mGameData.maxStamina || 30;
        mGameData.currentStamina = Math.min(
            maxStamina,
            Math.max(0, (mGameData.currentStamina || 0) + Math.floor(amount))
        );
        if (mGameData.SaveStaminaData) mGameData.SaveStaminaData();
        GameState.stamina = mGameData.currentStamina;
        GameState.save();
