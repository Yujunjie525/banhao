...
export default class LevelSelectManager extends cc.Component {
    ...
    onLevelClick(level: number) {
        ...
        // 如果是第一关且剧情弹窗未显示过，显示剧情弹窗
        if (level === 1 && !mGameData.isStoryPopupShown()) {
            // 获取LoadManager实例
            const loadManager = cc.find('Canvas/LoadManager')?.getComponent('LoadManager');
            if (loadManager) {
                this.onBackClick();
                loadManager.initStoryPopup();
                mGameData.setStoryPopupShown(); // 记录为已显示
                return; // 剧情弹窗会负责加载游戏场景
            }
        }
