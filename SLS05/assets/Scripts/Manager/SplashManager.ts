...
export default class SplashManager extends cc.Component {
    ...
    async checkLoginStatus() {
        ...
        if (this.username && this.password) {
            ...
            if (globalRealName === 'true') {
                ...
                // 调用register方法获取最新数据
                try {
                    const result = await this.register("app.shenyuanlieshou", this.username, this.password, 2);
                    ...
                    // 进行防沉迷检查
                    try {
                        const antiAddictionResult = await this.PostBreathe("app.shenyuanlieshou", this.username);
    ...
    async onLoginClick() {
        ...
        try {
            console.log("开始登录...");
            const result = await this.register("app.shenyuanlieshou", username, password, 2);
            console.log("登录结果:", result);
    ...
    async onZhuceClick() {
        ...
        // 注册
        try {
            const result = await this.register("app.shenyuanlieshou", username, password, 1);
    ...
    async onRealNameClick() {
        ...
        try {
            console.log("开始实名认证...");
            const result = await this.realName("app.shenyuanlieshou", this.username, realname, idnum);
