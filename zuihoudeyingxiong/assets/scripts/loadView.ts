...
export class loadView extends Component {
    ...
    private reloadCurrentUserMaxJuli() {
        const scopedMaxJuliKey = getUserScopedKey(localData.maxJuli);
        let maxJuli = load(scopedMaxJuliKey, 1);
        if (maxJuli == null) {
            maxJuli = 0;
            save(scopedMaxJuliKey, maxJuli);
        }
        gameConfig.maxJuli = maxJuli;
    }

    private reloadCurrentUserRoleData() {
        const scopedSelectTypeKey = getScopedSelectTypeKey();
        let slectType = load(scopedSelectTypeKey, 1);
        if (slectType == null) {
            slectType = 1;
        ...
        const scopedPlayerListKey = getScopedPlayerListKey();
   ...
   */
            ...
            if (globalRealName === 'true') {
                ...
                try {
                    ...
                    if (result.data.hasOwnProperty('diamond')) {
                        // 将服务端返回的diamond作为初始金币数量
                        const goldValue = Number(result.data.diamond);
                        // 保存到本地存储
                        save(localData.jinbiNum, goldValue.toString());
                        // 更新gameConfig中的金币值
                        gameConfig.jinbiNum = goldValue;
                        console.log('初始金币数量设置成功:', goldValue);
                    }
                    ...
                    await this.restoreCloudUserData(this.username);
                    ...
                    try {
                        ...
                        this.reloadCurrentUserRoleData();
                        // 重新加载关卡数据
    ...
    async onLoginClick() {
        ...
        try {
            ...
            if (result.code === 0) {
                ...
                const isRealName = result.data.isrealname === 1 || result.data.is_real === 1;
                if (isRealName) {
                    await this.restoreCloudUserData(username);
                    // 已实名，进行防沉迷检查
                    try {
    ...
    private async restoreCloudUserData(username: string) {
        try {
            const hasRemoteData = await cloudUserData.ins.pullAndApply(gameConfig.APP_ID, username);
            if (!hasRemoteData) {
                await cloudUserData.ins.pushCurrentUserData(gameConfig.APP_ID, username);
            }
        } catch (error) {
