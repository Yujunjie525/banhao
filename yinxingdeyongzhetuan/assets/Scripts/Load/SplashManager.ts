...
export default class SplashManager extends cc.Component {
    ...
    onLoad() {
        const eventRoot = this.node.parent;
        if (eventRoot) {
            eventRoot.on(cc.Node.EventType.TOUCH_END, this._playButtonClick, this);
        }
        [this.loginButton, this.zhuceBtn, this.realnameButton, this.btn_tc, this.BtnTips, this.BtnCloseTips].forEach((node: cc.Node) => {
            if (node && !node.getComponent(cc.Button)) node.addComponent(cc.Button);
        });
        // 检查本地存储，保存账号信息
    ...
    onDestroy() {
        const eventRoot = this.node.parent;
        if (eventRoot) {
            eventRoot.off(cc.Node.EventType.TOUCH_END, this._playButtonClick, this);
        }
    }
            ...
            if (globalRealName === 'true') {
                ...
                try {
                    ...
                    if (result.data.hasOwnProperty('diamond')) {
                        ...
                        console.log('登录返回的金币数量:', result.data.diamond);
                    }
                    ...
                    try {
                        ...
                        if (antiAddictionResult.data && antiAddictionResult.code === -1) {
                            TipsWndManager.show(
                                (antiAddictionResult.msg + '') || '未成年用户禁止进入游戏。',
                                0,
                                () => this.handleAntiAddictionBlocked()
                            );
                            return; // 阻止进入游戏)
                        }
    ...
    private handleAntiAddictionBlocked() {
        // The splash has already hidden the login UI during automatic login.
        // Remove the saved credentials so the next launch cannot re-enter this blocked path.
