...
class GameData{
    ...
    public bestDistanceKey:string = 'BestDistance';
    ...
    //金币系统相关
    public currentGold:number = 0; //当前金币数量，初始给1000
    public goldKey:string = 'CurrentGold'; //存储金币的key
    public totalGoldEarned:number = 0; //累计获得金币数量
    public totalGoldEarnedKey:string = 'TotalGoldEarned'; //存储累计金币的key
    ...
    public roleNames: Array<string> = ['秦甲先锋', '疾影游侠', '聚宝术士', '引磁猎手', '裂界行者']; //角色名称
...
private static LEVEL_CONFIGS: LevelConfig[] = [
     ...
     * 存储金币数据
     */
    SaveBestDistanceData(){
        const distance = Math.max(0, Math.floor(Number(this.bestDistance || this.BestScore) || 0));
        this.bestDistance = distance;
        this.BestScore = distance;
        cc.sys.localStorage.setItem(this.getKeyWithUserId(this.bestDistanceKey), distance.toString());
        cc.sys.localStorage.setItem(this.getKeyWithUserId(this.BestScoreKey), distance.toString());

        if(cc.sys.platform == cc.sys.WECHAT_GAME){
            (window as any).wx.setUserCloudStorage({
                KVDataList: [{ key: '1', value: distance.toString() }],
            });
        }

        UserDataSyncManager.requestUpload();
        UserDataSyncManager.flushUpload().catch((error) => {
            console.error('Upload best distance failed:', error);
        });
    }

    GetBestDistanceData(){
        const distanceStr = cc.sys.localStorage.getItem(this.getKeyWithUserId(this.bestDistanceKey))
            || cc.sys.localStorage.getItem(this.getKeyWithUserId(this.BestScoreKey));
        const distance = distanceStr ? parseInt(distanceStr, 10) : 0;
        this.bestDistance = isNaN(distance) ? 0 : Math.max(0, distance);
        this.BestScore = this.bestDistance;
    }

    updateBestDistance(distance: number): boolean {
        const nextDistance = Math.max(0, Math.floor(Number(distance) || 0));
        const currentDistance = Math.max(0, Math.floor(Number(this.bestDistance || this.BestScore) || 0));
        if (nextDistance <= currentDistance) {
            return false;
        }
        this.bestDistance = nextDistance;
        this.SaveBestDistanceData();
        return true;
    }

    SaveGoldData(){
        // 使用Cocos Creator的本地存储接口保存金币数据
    ...
    GetGoldData(){
        ...
        if (totalGoldEarnedStr) {
            ...
            this.totalGoldEarned = this.currentGold; // 如果没有累计数据，初始化为当前金币数
        }
    }
