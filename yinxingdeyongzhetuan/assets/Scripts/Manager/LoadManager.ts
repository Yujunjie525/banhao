...
export default class LoadManager extends cc.Component {
    ...
    @property(cc.Label)
    gold_lb:cc.Label = null; //当前金币显示标签
    ...
    onLoad () {
        ...
        cc.director.preloadScene('WarriorRun');
    ...
    initStoryPopup(){
        ...
        // 初始化剧情文本数组（示例为十几行文本）
        this.storyLines = [
            "当深渊魔域的黑暗势力从熔岩魔堡倾巢而出，王国的边境便再无宁日。一边是灯火不息的繁荣圣城，一边是烈焰翻涌的熔岩魔堡，这条贯穿南北的黄金赛道，成了光明与黑暗之间最后的屏障。而你，正是英雄的勇者团的统领者。率领队伍向魔域深处突进吧，以光明能量召唤身怀绝技的英雄伙伴，在三条赛道间灵活闪避、收集王冠钻石壮大力量。每前进一步，都是向黑暗的一次反击；每集结一位勇者，都为文明多添一分希望。冲破魔域的层层封锁，守护王国最后的火种 —— 这场奔跑，注定不朽。",
        ];
    ...
    async OnOkTips(){
        ...
        if(this.currentRechargeOption){
            ...
            // // 显示充值成功提示
            // TipsManager.show(`兑换成功！获得${this.currentRechargeOption.diamonds}个金币。`);
    ...
    private _playBgm(){
        this._stopBgm();
        if (!mGameData.isBGMOn || !this.Bgm) return;
