...
export default class RankManager extends cc.Component {
    ...
    private getRankLevel(): number {
        ...
        const storedCurrentLevel = cc.sys.localStorage.getItem(currentLevelKey) || cc.sys.localStorage.getItem('CurrentLevel');
        const storedUnlockedLevel = cc.sys.localStorage.getItem(unlockedLevelKey) || cc.sys.localStorage.getItem('UnlockedLevel');
        ...
        return Math.max(1, currentLevel + 1, unlockedLevel);
    }
    ...
    private async initRankData(): Promise<void> {
        ...
        try {
            ...
            if (result.code === 0 && result.data) {
                ...
                const localRankLevel = this.getRankLevel();
                
                // 处理返回的数据，将其转换为符合要求的格式
                this.rankData = result.data.map((item: any, index: number) => {
                    const name = `玩家${item.accountId}`;
                    
                    const serverRankLevel = Math.max(1, Math.floor(Number(item.totalLoginNum) || 0) + 1);
    ...
    private addCurrentUserToRank(): void {
        // 从GameData获取真实用户数据
        const selfName = this.getSelfName();
        const selfLevel = this.getRankLevel();
