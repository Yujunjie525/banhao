...
interface RemoteUserDataPayload {
    ...
    levelStars?: number[];
    failedLevels?: number[];
    warriorRunInfiniteGuideCompleted?: number;
    warriorRunLevelGuideCompleted?: number;
}
...
export default class UserDataSyncManager {
    ...
    private static readonly WARRIOR_RUN_INFINITE_GUIDE_COMPLETED_KEY = "WarriorRunInfiniteGuideCompleted";
    private static readonly WARRIOR_RUN_LEVEL_GUIDE_COMPLETED_KEY = "WarriorRunLevelGuideCompleted";
    ...
    public static async postPassLevel(rank: number, star: number): Promise<any> {
        ...
        const payload = {
            ...
            star: Math.max(0, Math.floor(Number(star) || 0))
        };
    ...
    public static isWarriorRunGuideCompleted(infiniteMode: boolean): boolean {
        const key = infiniteMode
            ? this.WARRIOR_RUN_INFINITE_GUIDE_COMPLETED_KEY
            : this.WARRIOR_RUN_LEVEL_GUIDE_COMPLETED_KEY;
        return this.readNumber(key, 0) >= 1;
    }

    public static completeWarriorRunGuide(infiniteMode: boolean): void {
        const key = infiniteMode
            ? this.WARRIOR_RUN_INFINITE_GUIDE_COMPLETED_KEY
            : this.WARRIOR_RUN_LEVEL_GUIDE_COMPLETED_KEY;
        cc.sys.localStorage.setItem(this.getScopedKey(key), "1");
        this.requestUpload();
    }
    ...
    private static buildLocalPayload(): RemoteUserDataPayload {
        ...
        return {
            ...
            warriorRunInfiniteGuideCompleted: this.readNumber(this.WARRIOR_RUN_INFINITE_GUIDE_COMPLETED_KEY, 0) >= 1 ? 1 : 0,
            warriorRunLevelGuideCompleted: this.readNumber(this.WARRIOR_RUN_LEVEL_GUIDE_COMPLETED_KEY, 0) >= 1 ? 1 : 0
        };
    }
    ...
    private static applyPayloadToLocal(payload: RemoteUserDataPayload): void {
        ...
        this.mergeGuideCompletion(
            this.WARRIOR_RUN_INFINITE_GUIDE_COMPLETED_KEY,
            payload.warriorRunInfiniteGuideCompleted
        );
        this.mergeGuideCompletion(
            this.WARRIOR_RUN_LEVEL_GUIDE_COMPLETED_KEY,
            payload.warriorRunLevelGuideCompleted
        );
    ...
    private static mergeGuideCompletion(baseKey: string, remoteValue?: number): void {
        const remoteCompleted = typeof remoteValue === "number" && Number.isFinite(remoteValue) && remoteValue >= 1 ? 1 : 0;
        const localCompleted = this.readNumber(baseKey, 0) >= 1 ? 1 : 0;
        const mergedCompleted = Math.max(localCompleted, remoteCompleted);
        cc.sys.localStorage.setItem(this.getScopedKey(baseKey), mergedCompleted.toString());
        if (mergedCompleted > remoteCompleted) {
            this.hasDirtyChanges = true;
        }
    }
