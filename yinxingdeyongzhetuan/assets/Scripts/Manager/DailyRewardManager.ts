...
export default class DailyRewardManager extends cc.Component {
    ...
    @property
    listVerticalSpacing: number = 34;

    @property
    listTopPadding: number = 12;
    ...
    private setNodeSprite(node: cc.Node, resourceName: string): void {
        ...
        cc.loader.loadRes(`2Main/${resourceName}`, cc.SpriteFrame, (err: Error, spriteFrame: cc.SpriteFrame) => {
            if (err) {
                console.error(`Load 2Main/${resourceName} failed:`, err);
                return;
            }
