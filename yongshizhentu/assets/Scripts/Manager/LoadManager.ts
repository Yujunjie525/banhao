...
import { resetProgressState } from "../game/GameState";
...
export default class LoadManager extends cc.Component {
    ...
    private isStartingEndlessRun: boolean = false;
    ...
    private _canvasClickTarget: cc.Node = null;
    ...
    @property(cc.Label)
    cur_level:cc.Label = null; //主界面最远距离显示标签
    ...
    onLoad () {
        // 主界面 BGM 由 audioEngine 的 music 通道独占，切场景时不会遗留旧曲目。
        cc.audioEngine.stopMusic();
        if (mGameData.isBGMOn && this.Bgm) {
            cc.audioEngine.playMusic(this.Bgm, true);
        }

        this._canvasClickTarget = this.node && this.node.parent;
        if (this._canvasClickTarget) {
            this._canvasClickTarget.on(cc.Node.EventType.TOUCH_END, this._playButtonClick, this);
        }
        ...
        if(mGameData.shouldOpenLevelSelect){
            ...
            this.scheduleOnce(() => this.openLevelSelectPanel(), 0);
        }
    ...
    start(){
        // Start is already visible here, so this preloads gameplay art without delaying the login/startup flow.
        WarriorRunAssetLoader.preload();
    }

    onDestroy(){
        if (this._canvasClickTarget) {
            this._canvasClickTarget.off(cc.Node.EventType.TOUCH_END, this._playButtonClick, this);
            this._canvasClickTarget = null;
        }
        cc.audioEngine.stopMusic();
    ...
    updateLevelDisplay(){
        if(this.cur_level){
            ...
            const rawDistance = cc.sys.localStorage.getItem(scopedKey) || cc.sys.localStorage.getItem('WarriorRunBestDistance');
            const bestDistance = Math.max(0, Math.floor(Number(rawDistance) || 0));
            this.cur_level.string = '最远距离：' + bestDistance + '米。';
        } else {
     ...
     */
    ResetLevel(){        
        StateBridge.resetLevelProgress();
        // 更新主界面关卡显示
        this.updateLevelDisplay();
        
        // 刷新关卡选择界面（如果存在）
        if (this.levelSelectPanel) {
            const levelSelectManager = this.levelSelectPanel.getComponent('LevelSelectManager') as any;
            if (levelSelectManager && levelSelectManager.initLevelList) {
    ...
    startEndlessRun(){
        ...
        this.isStartingEndlessRun = true;
        this.setStartButtonsInteractable(false);
        WarriorRunAssetLoader.preload(() => {
            ...
            if(!StateBridge.consumeStamina()){
                this.isStartingEndlessRun = false;
                this.setStartButtonsInteractable(true);
                TipsManager.show('体力不足，无法开始游戏。');
                return;
            }
    ...
    private setStartButtonsInteractable(interactable: boolean){
        if(this.BtnStart){
            this.BtnStart.interactable = interactable;
        }
        if(this.BtnStart2){
            this.BtnStart2.interactable = interactable;
        }
    }

    openLevelSelectPanel(){
        if(!this.levelSelectPanel){
            TipsManager.show('关卡选择界面未配置。');
            return;
        }
        const levelSelectManager = this.levelSelectPanel.getComponent('LevelSelectManager') as any;
        if(levelSelectManager && levelSelectManager.show){
            levelSelectManager.show();
        }else{
    ...
    ShowShop(){
        if(this.shopPanel){
            const shopManager = this.shopPanel.getComponent('ShopManager') as any;
    ...
    ShowRank(){
        if(this.rankPanel){
            const rankManager = this.rankPanel.getComponent('RankManager') as any;
    ...
    private resetRuntimeAccountState(){
        UserDataSyncManager.resetForAccountSwitch();
        resetProgressState();
        mGameData.currentLevel = 1;
        mGameData.unlockedLevel = 1;
        mGameData.levelStars = [];
        mGameData.isInfiniteMode = true;
    }
    ...
    initStoryPopup(){
        ...
        // 初始化剧情文本数组（示例为十几行文本）
        this.storyLines = [
            "曾几何时，这片广袤的大陆在水、火、草三大元素之力的均衡下安宁祥和。然而，一场名为“虚空狂潮”的异界灾乱凭空爆发，混乱的能量让原本温顺的元素生灵化作潮水般的魔物，盘踞在通往圣殿的古老遗迹大道上。曾经守护四方的远征卫队在突袭中被打散，散落在大道各处。如今，作为被选中的指挥官，你即将踏上这条被称为“勇士征途”的试炼之路。前路被层层魔物围堵，但你并非孤身一人。遗迹的三条战道上，残留着古老的神圣光环——只要在冲锋中精准穿过这些时空传送阵，就能不断唤醒并集结失散的卫队伙伴。面对不同属性的魔物首领，你需要洞悉元素相生相克的古老法则，带领不断壮大的小队在战道间灵活穿梭。前行不只为自保求生，更为重聚希望。在这场奇幻的冒险中，握紧武器，用智慧与勇气劈开黑暗，走完属于你的勇士征途！",
        ];
    ...
    private _playButtonClick(event: cc.Event.EventTouch){
        ...
        const canvas = this._canvasClickTarget;
    ...
    ShowQH(){
        ...
        if (this.SkillPanel) {
            const skillManager = this.SkillPanel.getComponent('SkillManager') as any;
    ...
    ShowWeek(){
       if(this.WeekPanel){
            const weeklyRewardManager = this.WeekPanel.getComponent('WeeklyRewardManager') as any;
    ...
    ShowDaily(){
       if(this.DailyPanel){
            const dailyRewardManager = this.DailyPanel.getComponent('DailyRewardManager') as any;
