...
export default class StateBridge {
    ...
    static resetLevelProgress(): void {
        loadProgress();
        const p = getProgress();
        p.unlocked_level = 1;
        p.level_stars = {};
        ...
        for (let level = 1; level <= totalLevels; level++) {
            cc.sys.localStorage.removeItem('LevelStars_' + level + suffix);
            cc.sys.localStorage.removeItem('WarriorRunLevelCleared' + suffix + '_' + level);
        }

        saveProgress();
        if (mGameData.SaveLevelData) mGameData.SaveLevelData();
        requestUpload();
        cc.director.emit('levelProgressReset');
    }
