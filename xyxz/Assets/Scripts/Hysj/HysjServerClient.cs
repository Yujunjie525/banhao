...
namespace Hysj
{
    ...
    {
        public const string AppId = "yongshigame014";
        ...
        private IEnumerator FetchUserData(bool preferNewest, Action<bool, bool, string> completed)
        {
            ...
            yield return Post("GetUserData", JsonUtility.ToJson(body), (ok, response) =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(payload))
                {
                    ...
                    {
                        // A malformed/legacy remote jsondata must not pollute gameplay with a console error.
                        // Keep the local save and let the normal sync result flow decide whether to upload it.
                        localUploadRequired = preferNewest;
                    }
                }
