import { _decorator, Color, Component, director, Graphics, Node, tween, Tween, UIOpacity, UITransform, Vec3 } from 'cc';
...
export class BlackHole extends Component {
    ...
    private checkPatrolCollision(): boolean {
        return this.grid.getLightBlocks().some((block) => {
            const isHit = !block.isSlotted && this.grid.sameCoord(block.getVisionCoord(), this.gridCoord);
            if (isHit && block.isSliding) {
                block.stopBeforeBlackHole(this.gridCoord);
            }
            return isHit;
        });
    }
        ...
        for (let dc = -1; dc <= 1; dc++) {
            for (let dr = -1; dr <= 1; dr++) {
                ...
                if (hitBlock && !this.caughtThisPulse.has(hitBlock.blockId)) {
                    if (hitBlock.isSliding) {
                        hitBlock.stopBeforeBlackHole(checkCoord);
                    }
    ...
    private updatePulseVisual(force = false) {
        ...
        const scale = this.isInPulse ? this.pulseScale : 1;
        tween(this.node)
