...
import { getStorageKey, load, save } from '../script/utils/tools';
import { cloudUserData } from '../script/utils/cloudUserData';
import { gameConfig } from '../script/data/gameConfig';
import OnlineTimeManager from '../script/view/OnlineTimeManager';
...
export class loadView extends Component {
    ...
    private readonly STORAGE_KEY_NEW_REGISTER_USERNAME = 'SLS_NEW_REGISTER_USERNAME';
   ...
   */
    ...
    async checkLoginStatus() {
        ...
        if (this.username && this.password) {
            ...
            if (globalRealName === 'true') {
                ...
                // 调用register方法获取最新数据
                try {
                    const result = await this.register(gameConfig.APP_ID, this.username, this.password, 2);
                    ...
                    // 进行防沉迷检查
                    try {
                        const antiAddictionResult = await this.PostBreathe(gameConfig.APP_ID, this.username);
                        ...
                        // 切换账号时清除旧数据并从服务端拉取
                        await this.switchAccountAndInit();
                        // 重新加载当前账户的体力数据
                        mGameData.ReloadStamina();
                        OnlineTimeManager.initialize();
                        // 如果后端没有返回关卡数据，才从本地存储加载
                        // 这样可以确保新角色不会继承旧角色的数据
                        if (!result.data.hasOwnProperty('rank')) {
    ...
    async onLoginClick() {
        ...
        try {
            console.log("开始登录...");
            const result = await this.register(gameConfig.APP_ID, username, password, 2);
                ...
                if (isRealName) {
                    ...
                    try {
                        ...
                        if (antiAddictionResult.data && antiAddictionResult.code === -1) {
                            ...
                            // TipsWndManager.show((antiAddictionResult.msg + '') || '未成年用户禁止进入游戏。');
                            loadPool.ins.getPoolNode('TipsWnd', this.node)
                            director.emit(emits.tipWndMsg, (antiAddictionResult.msg + '') || '未成年用户禁止进入游戏。', true)
                            return; // 阻止进入游戏
                        }
    ...
    async onZhuceClick() {
        ...
        // 注册
        try {
            const result = await this.register(gameConfig.APP_ID, username, password, 1);
            if (result.code === 0) {
                save(this.STORAGE_KEY_NEW_REGISTER_USERNAME, username);
            }
    ...
    async onRealNameClick() {
        ...
        try {
            console.log("开始实名认证...");
            const result = await this.realName(gameConfig.APP_ID, this.username, realname, idnum);
            ...
            if (result.code === 0) {
                ...
                try {
                    const antiAddictionResult = await this.PostBreathe(gameConfig.APP_ID, this.username);
                    console.log("realname breathe check:", antiAddictionResult);
                    ...
                    console.error("realname breathe check failed:", error);
                }
            } else {
                // 实名认证失败
                // TipsManager.show(result.msg + '');
     ...
     * 切换账号时清除旧数据并初始化新账号数据
     */
    private async switchAccountAndInit() {
        this.clearOldUserData();
        const isNewRegisteredAccount = load(this.STORAGE_KEY_NEW_REGISTER_USERNAME, 0) === this.username;
        if (isNewRegisteredAccount) {
            this.clearCurrentUserScopedData();
            this.initHome2Data();
            await cloudUserData.ins.pushCurrentUserData(gameConfig.APP_ID, this.username);
            sys.localStorage.removeItem(this.STORAGE_KEY_NEW_REGISTER_USERNAME);
            return;
        }
        // 先尝试从服务端拉取数据
        const hasRemoteData = await this.restoreCloudUserData(this.username);
        if (!hasRemoteData) {
            this.clearCurrentUserScopedData();
        }
        // 然后初始化数据（如果服务端没有数据，会使用默认值）
        this.initHome2Data();
        if (!hasRemoteData) {
            await cloudUserData.ins.pushCurrentUserData(gameConfig.APP_ID, this.username);
        }
    ...
    private clearCurrentUserScopedData() {
        const keysToClear = [
            localData.jinbiNum,
            localData.lightCrystal,
            localData.memoryFragment,
    ...
    private async restoreCloudUserData(username: string) {
        try {
            return await cloudUserData.ins.pullAndApply(gameConfig.APP_ID, username);
        } catch (error) {
            console.error('restoreCloudUserData failed:', error);
            return false;
        }
    }
