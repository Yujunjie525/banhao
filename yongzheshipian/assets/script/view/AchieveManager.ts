...
export default class AchieveManager extends Component {
    ...
    private checkAchieveCompletion(achieve: AchievementConfig) {
        switch (achieve.type) {
            case '1':
                return this.getHighestScore() >= achieve.count;
            case '2':
    ...
    private adjustContainerSize() {
        ...
        const horizontalSpacing = 30; // 两个列表项之间的间隔（可调整）
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const x = (col * (itemWidth + actualHorizontalSpacing)) - (viewTransform.width / 2) + itemWidth / 2 + 25;
