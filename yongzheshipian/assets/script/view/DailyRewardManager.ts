...
export default class DailyRewardManager extends Component {
    ...
    private readonly claimedButtonSpritePath = 'UI/ZZImg/anniu7/spriteFrame';
    private readonly lockedButtonSpritePath = 'UI/ZZImg/anniu11/spriteFrame';
    ...
    private adjustContainerSize() {
        ...
        const columnCount = Math.max(1, Math.floor(this.itemsPerRow));
        const horizontalSpacing = Math.max(0, this.horizontalSpacing);
        const verticalSpacing = Math.max(0, this.verticalSpacing);
        const horizontalPadding = Math.max(0, this.horizontalPadding);
        const verticalPadding = Math.max(0, this.verticalPadding);
        const rowCount = Math.ceil(children.length / columnCount);
        const availableWidth = Math.max(0, viewTransform.width - 2 * horizontalPadding);
        const totalSpacingWidth = horizontalSpacing * Math.max(0, columnCount - 1);
        const widthPerItem = columnCount > 0 ? (availableWidth - totalSpacingWidth) / columnCount : availableWidth;
        const itemScale = itemWidth > 0 ? Math.min(1, Math.max(0, widthPerItem / itemWidth)) : 1;
        const scaledItemWidth = itemWidth * itemScale;
        const scaledItemHeight = itemHeight * itemScale;
        const contentWidth = columnCount * scaledItemWidth + totalSpacingWidth;
        const totalHeight = rowCount * scaledItemHeight + Math.max(0, rowCount - 1) * verticalSpacing + 2 * verticalPadding;
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const x = -contentWidth / 2 + scaledItemWidth / 2 + column * (scaledItemWidth + horizontalSpacing);
            const y = -verticalPadding - scaledItemHeight / 2 - row * (scaledItemHeight + verticalSpacing);
            child.setScale(new Vec3(itemScale, itemScale, 1));
